    ...
    <div className="space-y-6">
      <PageHeader
        breadcrumbs={[
          ...
          { label: "Candidatos", href: "/panel/candidates" },
          { label: candidate.name || "Sin nombre" },
        ]}
        backHref="/panel/candidates"
      />
      {/* Header */}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="border border-base-300 bg-base-100 rounded-lg p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {candidate.email && (
                <div>
                  <span className="text-[11px] text-base-content/60 uppercase tracking-wider">Email</span>
                  <p className="text-sm text-base-content mt-0.5">{candidate.email}</p>
                </div>
              )}
              {candidate.phone && (
                <div>
                  <span className="text-[11px] text-base-content/60 uppercase tracking-wider">Teléfono</span>
                  <p className="text-sm text-base-content mt-0.5">{candidate.phone}</p>
                </div>
              )}
              {candidate.address && (
                <div className="sm:col-span-2">
                  <span className="text-[11px] text-base-content/60 uppercase tracking-wider">Dirección</span>
          ...
          {candidate.cvSummary && (
            <div className="border border-base-300 bg-base-100 rounded-lg p-5">
              ...
              <p className="text-sm text-base-content/80 leading-relaxed">{candidate.cvSummary}</p>
            </div>
          )}
        ...
        <div className="space-y-6">
          ...
          <div className="border border-base-300 bg-base-100 rounded-lg p-5">
            ...
            <div className="flex items-center gap-2">
              <span className={`badge badge-lg ${statusColors[candidate.status] ?? "badge-ghost"}`}>
                {statusLabels[candidate.status] ?? candidate.status}
              </span>
              <span className="text-[11px] text-base-content/50">
                (derivado de sus vacantes)
              </span>
            </div>
          </div>
          ...
          <VacanciesCard
            ...
            initialVacancies={candidate.vacancies.map((cv) => ({
          ...
          {candidate.rawText && (
            <div className="border border-base-300 bg-base-100 rounded-lg p-5">
              <details>
                <summary className="text-xs font-semibold text-base-content/70 uppercase tracking-widest cursor-pointer flex items-center gap-2">
                  <span className="icon-[tabler--code] size-4 text-primary" />
                  Texto extraído
                </summary>
                <pre className="mt-3 text-xs text-base-content/75 whitespace-pre-wrap max-h-96 overflow-y-auto bg-base-200 rounded-lg p-3">
                  {candidate.rawText}
                </pre>
              </details>
            </div>
